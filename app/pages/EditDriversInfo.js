...
import {useRegisterDriverMutation, useUpdateDriversInfoMutation} from 'api';
...
const StepTwo = ({formik, step, data}) => {
  if (step !== 1) {
    return null;
  }
  return (
    <View className="w-full h-[5rem] ">
      {registrationData?.slice(5, 12)?.map((v, i) => (
        <View key={i + 1}>
          <Input
            label={v.label}
            placeholder={v.placeholder}
      ...
      <SelectInput
        ...
        defaultButtonText={data?.location?.state}
      ...
      <View className="mt-2">
        <SelectInput
          ...
          defaultValue={data?.meta?.bankName}
          defaultButtonText={data?.meta?.bankName}
          label="Select bank"
        />
      </View>
    </View>
...
const labels = ['Profile', 'other info'];
...
export default function EditDriversInfo(props) {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      location: {
        state: driversData?.location?.state,
      },
      meta: {
        bankName: driversData?.meta?.bankName,
        accountNumber: driversData?.meta?.accountNumber,
        // driverLicenseNumber: '4390365819',
        carBrand: driversData?.meta?.carBrand,
        carModel: driversData?.meta?.carModel,
        carColor: driversData?.meta?.carColor,
      },
      kyc: {
        meansOfIdentification: driversData?.kyc?.meansOfIdentification,
        identificationNumber: driversData?.kyc?.identificationNumber,
        identificationImage:
      ...
      <ScreenWrapper backBtn={true} navigation={props.navigation}>
        ...
        <View style={styles.inner}>
          ...
          <ScrollView
            ...
            <StepTwo formik={formik} step={step} data={driversData} />
          </ScrollView>
          <View className="mt-5 flex justify-evenly">
            <Button
              name={step < 1 ? 'Next' : 'Submit'}
